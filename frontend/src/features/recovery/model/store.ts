...
import { recoveryApi } from '@/features/recovery/model/api'
import type { RecoveryTaskVO } from '@/features/recovery/model/types'
...
export const useRecoveryStore = defineStore('recovery', () => {
  ...
  const completedTasks = computed(() => tasks.value.filter((t) => isTerminal(t.status)))
