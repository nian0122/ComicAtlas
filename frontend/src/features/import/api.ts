...
export const importApi = {
  create: (sourceType: string, sourceValue: string) =>
    api.post<ImportTaskVO>('/manage/tasks/import', {
      sourceType,
      ...(sourceType === 'EHENTAI' ? { sourceRef: sourceValue } : { sourcePath: sourceValue }),
    }),
  ...
  createBatch: (data: BatchImportRequest) => api.post<BatchImportResultVO>('/manage/tasks/import/batch', data),
}

export const directoryScanApi = {
  create: (parentPath: string) => api.post<DirectoryScanTaskVO>('/manage/tasks/directory-scan', { parentPath }),
