...
import { isBlockingScanWarning } from '@/features/import/model/types'
import type { ScanItemVO, ScanPreviewNodeVO, ScanResultVO, ScanWarningVO } from '@/features/import/model/types'
...
export function useImportScan(scan: (path: string) => Promise<ScanResultVO>) {
  ...
  const importableCount = computed(() => (scanResult.value?.items ?? []).filter(isImportable).length)
