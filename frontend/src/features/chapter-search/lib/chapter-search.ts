...
export function searchChapters(index: readonly ChapterSearchItem[], keyword: string): ChapterSearchResult[] {
...
function filterNode(
  ...
  const chapters = catalogMatches
    ...
    : node.chapters.filter((chapter) =>
        normalizeText([chapterLabel(chapter), chapter.chapterNo, chapter.title, ...nextCatalogPath].join(' ')).includes(
          normalizedKeyword,
        ),
      )
  ...
  node.children.forEach((child, index) => {
    ...
    const childResult = catalogMatches
      ? {
          node: child,
          results: collectResultItems(child, nextCatalogPath),
          expandedNodePaths: collectNodePaths(child, childPath),
        }
...
function collectNodePaths(node: CatalogNode, nodePath: string): string[] {
  ...
  return node.children
    .flatMap((child, index) => collectNodePaths(child, `${nodePath}/${keySegmentOf(child, index)}`))
    .concat(paths)
}
