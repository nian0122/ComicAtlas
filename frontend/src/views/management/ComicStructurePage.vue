...
async function loadTree(): Promise<void> { loading.value = true; error.value = ''; try { tree.value = ((await catalogApi.tree(comicId.value)).data || []) as CatalogNode[] } catch (reason: unknown) { error.value = errorMessage(reason) } finally { loading.value = false } }
