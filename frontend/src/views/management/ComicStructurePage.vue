      ...
      <aside class="action-panel">
        ...
        <template v-else-if="selectedMediaIds.length">
          <div class="action-card action-card--media-batch">
            <div class="action-card-head"><div><span class="panel-kicker">BATCH MEDIA MAINTENANCE</span><h2>批量媒体操作</h2><p>仅处理当前章节中勾选的媒体。</p></div><span class="action-id">{{ selectedMediaIds.length }} ITEMS</span></div>
            <div class="batch-selection-summary"><strong>{{ selectedMediaIds.length }}</strong><span>个媒体已选择</span></div>
...
import { catalogManagementApi, chapterManagementApi, managementCatalogApi, managementChapterApi, mediaManagementApi } from '@/features/comic/management-api'
...
function handleMediaSelection(rows: MediaItemInfo[]): void { selectedMediaIds.value = rows.map((item) => item.id) }
function clearMediaSelection(): void { selectedMediaIds.value = []; mediaTableRef.value?.clearSelection() }
...
async function loadMedia(): Promise<void> { try { mediaItems.value = (await managementChapterApi.detail(mediaChapterId.value)).data.pages; mediaOrderItems.value = [...mediaItems.value]; mediaOrder.value = mediaItems.value.map((item) => item.id).join(','); selectedMedia.value = null; selectedMediaIds.value = []; mediaTableRef.value?.clearSelection() } catch (reason: unknown) { ElMessage.error(errorMessage(reason)) } }
...
.issue-strip { display: flex; align-items: center; gap: var(--space-4); min-width: 0; overflow: hidden; padding: 9px 12px; border: 1px solid color-mix(in srgb, var(--warning) 45%, var(--border)); border-left: 3px solid var(--warning); border-radius: var(--radius-sm); background: color-mix(in srgb, var(--warning) 5%, var(--bg-surface)); }
...
.issue-chapter-list { display: flex; flex: 1 1 auto; min-width: 0; gap: 5px; overflow-x: auto; overflow-y: hidden; scrollbar-color: var(--border-strong) var(--bg-surface); scrollbar-width: thin; }.issue-chapter { display: grid; flex: 0 0 150px; gap: 2px; min-width: 120px; padding: 6px 8px; border: 1px solid var(--border); border-radius: var(--radius-xs); background: var(--bg-elevated); color: var(--text-primary); text-align: left; cursor: pointer; }.issue-chapter:hover { border-color: var(--accent); background: var(--accent-bg); }.issue-chapter span { overflow: hidden; font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }.issue-chapter small { overflow: hidden; color: var(--warning); font-size: 9px; text-overflow: ellipsis; white-space: nowrap; }
...
.media-selection-toolbar { display: flex; align-items: center; justify-content: space-between; gap: var(--space-3); margin: 0 0 var(--space-2); padding: 7px 10px; border: 1px solid var(--accent-border); background: var(--accent-bg); color: var(--accent); font-size: 11px; }.media-selection-toolbar > div { display: flex; align-items: center; gap: 4px; }.media-selection-toolbar .el-button { margin: 0; padding: 3px 6px; font-size: 10px; }
