    ...
    <div class="filters">
      <el-select v-model="query.type" placeholder="任务类型" clearable @change="resetAndLoad">
        <el-option
          ...
          :key="type"
          :label="managementTaskTypeLabel(type)"
          :value="type"
        />
      </el-select>
          ...
          <article v-for="task in group.tasks" :key="task.id" class="task-card" @click="openTask(task)">
            ...
            <div class="task-card-body">
              ...
              <div class="task-card-footer">
                <span
                  >成功 {{ task.successCount ?? 0 }} · 失败 {{ task.failureCount ?? 0 }} · 取消
      ...
      <div v-if="selectedTask" class="task-detail">
        ...
        <section class="detail-progress-panel" aria-label="任务进度">
          ...
          <div class="detail-stat-grid">
            <div>
              <span>总量</span><strong>{{ selectedTask.totalCount ?? 0 }}</strong>
            </div>
            <div>
              <span>成功</span><strong>{{ selectedTask.successCount ?? 0 }}</strong>
            </div>
            <div>
              <span>失败</span><strong>{{ selectedTask.failureCount ?? 0 }}</strong>
            </div>
            <div>
              <span>取消</span><strong>{{ selectedTask.cancelledCount ?? 0 }}</strong>
            </div>
          </div>
        </section>
        ...
        <section class="detail-section">
          ...
          <div class="detail-fields">
            ...
            <div>
              <span>目标 ID</span><strong>{{ selectedTargetId ?? '—' }}</strong>
            </div>
            <div>
              <span>目标类型</span><strong>{{ selectedTask.targetType }}</strong>
            </div>
            <div>
              <span>漫画名称</span><strong>{{ taskDisplayName(selectedTask) }}</strong>
            </div>
            <div>
              <span>尝试次数</span><strong>{{ selectedTask.attempt ?? 0 }}</strong>
            </div>
            <div>
              <span>批次 ID</span><strong>{{ selectedTask.batchId || '—' }}</strong>
            </div>
          </div>
        </section>
        ...
        <section class="detail-section">
          ...
          <div class="detail-fields detail-fields--times">
            <div>
              <span>创建时间</span><strong>{{ formatDetailTime(selectedTask.createdAt) }}</strong>
            </div>
            <div>
              <span>开始时间</span><strong>{{ formatDetailTime(selectedTask.startedAt) }}</strong>
            </div>
            <div>
              <span>最近更新</span><strong>{{ formatDetailTime(selectedTask.updatedAt) }}</strong>
            </div>
            <div>
              <span>完成时间</span><strong>{{ formatDetailTime(selectedTask.completedAt) }}</strong>
            </div>
          </div>
        </section>
        ...
        <section class="detail-section">
          ...
          <el-table v-if="taskItems.length" :data="taskItems" row-key="id" class="detail-items-table">
            ...
            <el-table-column label="状态" min-width="100"
              ><template #default="{ row }"><TaskStatusTag class="task-status-tag" :status="row.status" /></template
            ></el-table-column>
            <el-table-column label="进度" min-width="120"
              ><template #default="{ row }">{{ row.progress ?? 0 }}%</template></el-table-column
            >
...
const groupedTasks = computed(() =>
  MANAGEMENT_TASK_TYPES.map((type) => ({
    type,
    label: managementTaskTypeLabel(type),
    tasks: tasks.value.filter((task) => task.taskType === type),
...
function taskStatusLabel(status: ManagementTaskStatus): string {
  return managementTaskStatusLabel(status)
}
...
function applyTarget(): void {
  const parsed = Number(targetIdInput.value)
  query.targetId = Number.isSafeInteger(parsed) && parsed > 0 ? parsed : undefined
  resetAndLoad()
}
async function openTask(task: ManagementTaskVO): Promise<void> {
  selectedTask.value = task
  drawerVisible.value = true
  try {
    taskItems.value = (await managementTaskApi.getItems(task.id)).data
  } catch (reason: unknown) {
...
async function cancelTask(id: number): Promise<void> {
  try {
    await managementTaskApi.cancel(id)
...
async function retryTask(id: number): Promise<void> {
  try {
    await managementTaskApi.retry(id)
...
onMounted(() => {
  void loadTasks()
  timer = setInterval(() => {
    if (autoRefresh.value && !loading.value) void loadTasks(true)
  }, 2500)
})
onBeforeUnmount(() => {
...
.task-card-accent.status-succeeded {
  background: var(--success);
}
.task-card-accent.status-failed,
.task-card-accent.status-partially_succeeded {
  background: var(--danger);
}
.task-card-accent.status-queued,
.task-card-accent.status-running,
.task-card-accent.status-cancelling {
...
.detail-progress-heading strong {
  color: var(--text-primary);
  font-size: var(--text-section);
}
.detail-stat-grid {
