<template>
  <div class="media-upload-page">
    ...
    <section class="upload-workbench">
        ...
        <el-form label-position="top" @submit.prevent="startUpload">
          <el-form-item label="漫画" required>
            <el-select
              ...
              @change="onComicChanged"
            >
              <el-option v-for="comic in comics" :key="comic.id" :label="comic.title" :value="comic.id"
                ><span>{{ comic.title }}</span
                ><small> #{{ comic.id }}</small></el-option
              >
            </el-select>
          </el-form-item>
          ...
          <el-form-item v-if="mode === 'replace'" label="替换目标" required>
            <el-select
              v-model="replaceMediaId"
              filterable
              clearable
              placeholder="选择要替换的媒体"
      ...
      <div class="upload-file-card">
        ...
        <label class="dropzone" :class="{ 'is-ready': selectedFiles.length }">
          ...
          ><strong>{{ selectedFiles.length ? `已选择 ${selectedFiles.length} 个文件` : '选择或拖入媒体文件' }}</strong
          ><small>上传前会计算 SHA-256，分片大小由服务端返回</small>
        </label>
        <div v-if="selectedFiles.length" class="file-list" aria-live="polite">
          <div v-for="file in selectedFiles" :key="file.id" class="file-row">
            <div class="file-copy">
              <strong>{{ file.file.name }}</strong
              ><small>{{ formatBytes(file.file.size) }} · {{ file.status }}</small>
            </div>
    ...
    <section class="upload-guide">
      ...
      <ol>
        <li><strong>校验</strong><span>浏览器计算文件摘要，服务端检查目标章节和文件类型。</span></li>
...
import type { CatalogNode, ChapterRef, ComicListVO } from '@/entities/comic/model/types'
...
const canStart = computed(() =>
  Boolean(
    ...
    (mode.value === 'append' || replaceMediaId.value),
  ),
)

function flattenChapters(nodes: readonly CatalogNode[]): ChapterRef[] {
  return nodes
    .flatMap((node) => [...node.chapters, ...flattenChapters(node.children ?? [])])
    .sort((left, right) => left.globalOrder - right.globalOrder)
}
function chapterLabel(chapter: ChapterRef): string {
  return `${chapter.title || `章节 ${chapter.chapterNo}`} · ${chapter.pageCount} 个媒体`
}
function mediaLabel(media: MediaItemInfo): string {
  return `${media.pageNumber}. ${media.fileName || '未命名媒体'} · ${media.mediaType === 'VIDEO' ? '视频' : '图片'}`
}
...
async function createManifest(): Promise<readonly UploadFileManifest[]> {
  return Promise.all(
    selectedFiles.value.map(async (row) => {
      row.status = '计算校验值'
...
async function uploadRow(row: UploadRow, chunkSize: number): Promise<void> {
  let offset = 0
  row.status = '上传中'
  while (offset < row.file.size) {
    const end = Math.min(offset + chunkSize, row.file.size)
...
function onFilesChanged(event: Event): void {
  ...
  const files = Array.from(input.files ?? []).slice(0, mode.value === 'replace' ? 1 : undefined)
...
async function startUpload(): Promise<void> {
  if (!canStart.value || !selectedComicId.value || !selectedChapterId.value) return
  ...
  try {
    const request: CreateUploadSessionRequest = {
      ...
      ...(mode.value === 'replace' ? { replaceMediaId: replaceMediaId.value } : {}),
    ...
    const completed = (await uploadApi.completeSession(created.sessionId)).data
    taskId.value = completed.taskId
...
async function cancelUpload(): Promise<void> {
  ...
  if (sessionId.value) await uploadApi.cancelSession(sessionId.value)
  uploading.value = false
  uploadStatus.value = '已取消'
}
async function onComicChanged(): Promise<void> {
  selectedChapterId.value = undefined
  replaceMediaId.value = undefined
  mediaItems.value = []
  chapters.value = []
  if (!selectedComicId.value) return
  loadingChapters.value = true
  try {
    chapters.value = flattenChapters((await managementCatalogApi.tree(selectedComicId.value)).data)
  } catch (reason: unknown) {
    errorMessage.value = getApiErrorMessage(reason, '章节加载失败')
  } finally {
    loadingChapters.value = false
  }
}
async function onChapterChanged(chapterId?: number): Promise<void> {
  replaceMediaId.value = undefined
  mediaItems.value = []
  if (!chapterId) return
  loadingMedia.value = true
  try {
    mediaItems.value = ((await managementChapterApi.detail(chapterId)).data.pages ?? []) as MediaItemInfo[]
  } catch (reason: unknown) {
    errorMessage.value = getApiErrorMessage(reason, '媒体列表加载失败')
  } finally {
...
watch(mode, () => {
  replaceMediaId.value = undefined
  if (mode.value === 'replace' && selectedFiles.value.length > 1) selectedFiles.value = selectedFiles.value.slice(0, 1)
})
onMounted(async () => {
  loadingComics.value = true
  try {
    comics.value = (await managementComicApi.list({ size: 200, order: 'asc', sort: 'title' })).data.records
  } catch (reason: unknown) {
    errorMessage.value = getApiErrorMessage(reason, '漫画列表加载失败')
  } finally {
...
.dropzone {
  ...
  background: color-mix(in srgb, var(--color-brand) 4%, var(--bg-surface));
  cursor: pointer;
  text-align: center;
  transition:
...
/* 业务根类命中每个按钮组件的根节点，抵消 Element Plus 默认相邻按钮间距并交由 flex gap 统一布局。 */
