...
public final class MetadataRefreshLimits {
    ...
    /**
     * 快照产物落盘路径布局模板（相对 MANGA_ROOT）：{@code STAGING/metadata-refresh/{taskId}/{itemId}/{attempt}/snapshot.json}。
     * 变量由任务上下文替换；attempt 保证重试不覆盖上一次产物，便于对比与排查。
     */
    public static final String SNAPSHOT_PATH_TEMPLATE =
            "STAGING/metadata-refresh/{taskId}/{itemId}/{attempt}/snapshot.json";
}
