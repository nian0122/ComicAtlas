...
| 模块 | 包前缀 | 职责 | 禁止内容 |
|---|---|---|---|
| `comic-shared` 的 contract 包 | `com.comicatlas.contract` | 请求/响应 DTO、统一结果、异常、枚举、缓存名称 | Entity、Mapper、SQL、文件访问实现 |
...
依赖方向为：`api-service` / `reading-service` → `comic-shared`。`contract` 包不得反向引用 `persistence` 包。`comic-common` 继续只提供 MQ 事件契约和通用工具。
