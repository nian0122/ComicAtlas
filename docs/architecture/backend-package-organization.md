...
后续分类待办见 [后端文件分类 TODO](backend-classification-todo.md)。DDD 迁移目标与依赖方向见 [管理后端 DDD 目录与依赖规范](backend-ddd-management.md)。该清单仅标记待调整归属，不执行代码迁移；下文“本次归类”和验证记录描述此前已经完成的整理。
