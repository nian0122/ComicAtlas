...
检查日期：2026-09-16。本文件记录实现问题及整改结果；源码标记已移除，具体整改以当前代码和测试为准。

## 已修复

- IMPL-01：导入任务终态拒绝所有迟到状态写入，合法重试仍通过重试流程进入新执行状态。
- IMPL-02：任务项状态和进度更新使用 attempt、非终态条件更新，并依据受影响行数处理并发冲突。
- IMPL-03：恢复进度改用事件批次总数作为分母，并覆盖单本恢复异常的进度推进。
- IMPL-04：磁盘统计遇到读取/遍历异常时抛出带 cause 的业务异常，避免将失败结果缓存为零容量。
- IMPL-05：上传 complete 使用 VERIFYING 冻结会话，将文件校验移出落库事务；校验或提交失败时恢复 ACTIVE。
- IMPL-06：SHA-256 计算异常保留原始 cause，且对外错误信息不透传本地路径。
- IMPL-07：首次进入 RUNNING 时在同一条件更新中写入 `started_at`，重复 RUNNING 不覆盖原值。

以上修复已通过 `api-service` 主代码编译；完整测试当前受测试源码引用缺失的 Worker 辅助类阻断。

以下依据源码控制流和更新条件进行静态分析。列出的触发场景是后续回归要求，尚未为本轮问题新增测试或进行故障复现。已有测试通过不代表这些未覆盖场景正确。

## P1：状态一致性与事务

### IMPL-01：导入终态仍可被其他终态覆盖（已修复）
...
- 修复前证据：当前状态属于终态时，仅当新状态为空或不是终态才返回；若新状态也是终态，仍执行 `task.setStatus(mappedStatus)` 和 `updateById`。
...
### IMPL-02：任务状态和进度更新存在检查与写入之间的并发窗口（已修复）
...
- 修复前证据：先 `selectById` 检查 attempt 和终态，后续 `LambdaUpdateWrapper` 只以 ID 为条件；状态和 attempt 未进入该 UPDATE 的条件，也未依据更新行数处理竞态。
...
### IMPL-05：上传完成校验在数据库事务内读取全部文件（已修复）

位置：[UploadSessionService.complete](../../api-service/src/main/java/com/comicatlas/api/upload/service/UploadSessionService.java)。

- 修复前证据：`@Transactional` 方法先查会话和文件，再调用 `verifyUploadedFiles`；该方法对每个暂存文件计算完整 SHA-256 并检测媒体类型，之后才写 STAGING 媒体、任务与 Outbox。
...
- 修复前证据：进度公式为 `(recovered + skipped + placeholder + errors) * 100 / totalSoFar`。RecoveryEngine 每处理一本返回 `totalSoFar + 1`，各分类计数之和通常也等于已处理数量。
...
- 修复前证据：捕获 IOException 后仅把 `ex.getMessage()` 拼入新的 BusinessException，未传递 cause。
- 触发：打开或读取暂存文件失败。
...
### IMPL-07：首次 RUNNING 的开始时间未进入数据库更新（已修复）
...
- 修复前证据：当新状态为 RUNNING 且开始时间为空时，先调用 `item.setStartedAt(now)`；构造更新语句后，再次以相同的 `item.getStartedAt() == null` 条件决定是否写 `started_at`。此时该条件已不成立。
...
```powershell
rg -n 'IMPL-' api-service/src/main/java
```
