...
> 管理控制台（回收站、批量操作、任务中心）为 v1.0 新增；媒体上传/替换为预留接口能力（后端接口可用、当前无前端页面入口）。部署与数据库账号等运维细节见[部署运维](operations/management.md)。
...
打开漫画卡片进入详情页。详情页提供封面、元数据、标签、目录树和章节入口；管理后台可修改标题、日文标题、作者、描述、分类、标签以及封面候选，一次保存即同时生效（`DRAFT`/`READY` 状态可编辑，并发修改会提示重新加载）。
