...
import com.comicatlas.api.task.interfaces.rest.dto.batch.BatchSelectionVO;
...
class BatchOperationServiceTest {
    ...
    private BatchEligibilityChecker realChecker() {
        BatchPersistencePort persistencePort = mock(BatchPersistencePort.class);
        when(persistencePort.findComicStatus(anyLong())).thenReturn(Optional.of("READY"));
        ...
        return new BatchEligibilityChecker(persistencePort,
    ...
    void eligibilityChecker_metadataRefresh走资产资格_部分不可用() {
        ...
        BatchEligibilityChecker checker = new BatchEligibilityChecker(persistencePort,
                mock(OperationPolicyService.class), assetEligibility);
