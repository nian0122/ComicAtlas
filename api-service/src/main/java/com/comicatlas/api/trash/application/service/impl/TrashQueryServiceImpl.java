...
public class TrashQueryServiceImpl implements TrashQueryService {
    // 回收查询契约由应用服务公开，具体实现保持在回收业务包内。
    /** 分页大小上限，防止单次查询拉取过多数据。 */
    private static final int MAX_PAGE_SIZE = 100;

    private final TrashQueryPersistencePort trashQueryPersistencePort;
    ...
    public List<TrashContentVO> list(String status, String keyword, int page, int size) {
        ...
        return trashQueryPersistencePort.findPage(status, normalize(keyword), (safePage - 1) * safeSize, safeSize);
    }

    public long count(String status, String keyword) {
        Objects.requireNonNull(status, "status 不能为 null");
        return trashQueryPersistencePort.count(status, normalize(keyword));
    }
