...
public class ImportFinalizationService {
    ...
    public void applyCompleted(ImportStorageFinalizeCompletedEvent event) {
        ...
        transactionTemplate.executeWithoutResult(status -> {
            ...
            if (task == null || comic == null || TERMINAL.contains(task.getStatus())
                    || comic.getStatus() != ComicStatus.IMPORTING || event.chapterId() == null) return;
            ...
            for (Chapter chapter : chapters) if (chapter.getId().equals(event.chapterId())
            ...
            if (task.getStartTime() != null) task.setDurationMs(Duration.between(task.getStartTime(), task.getEndTime()).toMillis());
            ...
            if (item != null) managementTaskService.updateItemStatus(item.getId(), ManagementTaskStatus.SUCCEEDED,
    ...
    public void applyFailed(ImportStorageFinalizeFailedEvent event) {
        transactionTemplate.executeWithoutResult(status -> {
            ...
            ManagementTaskItem item = managementTaskService.findActiveItem(TARGET_TYPE_COMIC, event.comicId(), TaskType.IMPORT);
            if (item != null) managementTaskService.updateItemStatus(item.getId(), ManagementTaskStatus.FAILED,
    ...
    private String hqPrefix() {
        ...
        if (mangaRoot == null || mangaRoot.isBlank()) return hqPath.getFileName() == null ? DEFAULT_HQ_DIR : hqPath.getFileName().toString();
