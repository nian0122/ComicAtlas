...
public final class AutomaticMetadataClassifier {
     ...
     */
    public static Enrichment classify(String explicitCategory, List<CategorySnapshot> categories,
                                      String title, String titleJpn, String author,
                                      String description, List<String> importedTags) {
        List<CategorySnapshot> availableCategories = categories == null ? List.of() : categories;
        String explicitValue = normalize(explicitCategory);
        CategorySnapshot selectedCategory = findCategory(explicitValue, availableCategories);
        ...
        if (selectedCategory == null && explicitValue == null) {
            selectedCategory = availableCategories.stream()
                    .filter(category -> normalize(category.name()) != null)
                    .filter(category -> containsIgnoreCase(searchableText, category.name()))
                    .sorted(Comparator.comparingInt((CategorySnapshot category) -> normalize(category.name()).length())
                            .reversed())
                    .findFirst()
                    .orElse(null);
        }
    ...
    private static CategorySnapshot findCategory(String explicitCategory, List<CategorySnapshot> categories) {
        if (explicitCategory == null) {
            return null;
        }
        return categories.stream()
                .filter(category -> explicitCategory.equalsIgnoreCase(normalize(category.name())))
    ...
    /** 推断结果；分类为空表示没有可靠匹配。 */
    public record Enrichment(CategorySnapshot category, List<String> inferredTags, String inferredTagType) {
    }
}
